...
int	init_map(t_var *var, char *f_str)
{
	...
	sf_str = ft_split(f_str, '\n');
	start_line = search_map(sf_str);
	if (start_line > 0)
	{
		var->size_map.y = y_map(sf_str, start_line);
		if (var->size_map.y > 0)
			var->size_map.x = x_map(sf_str, start_line);
		if (var->size_map.x > 0)
			alloc_map(&(var->map2d), var->size_map);
	}
	else
		return (free_char_tab(sf_str), -6);
	if (create_map(var, sf_str, start_line) >= 0)
		check_map(var);
	else
		return (free_char_tab(sf_str), -7);
	free_char_tab(sf_str);
	if (!var->map2d)
		return (-6);
	return (0);
}
