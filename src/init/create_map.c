...
static void	init_player(t_var *var, int pos[], int c)
{
	var->player.pos.x = pos[2];
	var->player.pos.y = pos[0];
	var->map2d[pos[0]][pos[2]] = 0;
	var->player.pos_x = (var->player.pos.x * SIZESQUARE)
		+ (SIZESQUARE / 2) - 1;
	var->player.pos_y = (var->player.pos.y * SIZESQUARE)
		+ (SIZESQUARE / 2) - 1;
	if (c == 'N')
		var->player.a = 270.0f;
	if (c == 'S')
